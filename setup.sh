...
IPVPS=$(curl -sS https://raw.githubusercontent.com/Fahmi-XD/ip/refs/heads/main/ip.txt | grep $MYIP | awk '{print $4}')
if [[ $MYIP == $IPVPS ]]; then
domain
Pasang
else
...
function key2(){
    ...
    if [[ $key == "1" ]]; then
        ...
        mkdir /root/listip
        cd /root/listip
        wget https://raw.githubusercontent.com/Fahmi-XD/ip/refs/heads/main/ip.txt >/dev/null 2>&1
...
res7() {
wget https://raw.githubusercontent.com/Fahmi-XD/vmess/refs/heads/main/update.sh && chmod +x update.sh && ./update.sh
clear
}
